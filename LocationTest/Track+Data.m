...
#import "Segment.h"
#import "Waypoint.h"
#import "MKPolyline+EncodedPolyline.h"
...
- (NSOrderedSet *)points
{
	NSMutableOrderedSet *points = [NSMutableOrderedSet orderedSet];
	for (Segment *segment in self.segments) {
		[points addObjectsFromArray:[segment.points array]];
	}
	return points;
}
...
- (double *)computeDistancesForPoints:(NSArray *)points
{
	int maxLoc = 0;
	double maxDist = 0;
	double absMaxDist = 0;
	...
	float minDist = VERYSMALL * points.count / 100;
...
- (NSString *)encodedPolylineString
{
	...
	double *dists = [self computeDistancesForPoints:points];
...
{
	return [self.points.array componentsJoinedByString:@"|"];
}
